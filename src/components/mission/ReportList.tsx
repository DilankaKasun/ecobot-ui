...
const HEALTH_TONE: Record<string, string> = {
  ...
  unknown: 'text-muted-foreground border-gray-500/30 bg-gray-500/10',
};
  ...
  return (
    <div className="bg-card border border-card-border rounded-xl p-4 shadow-sm space-y-3">
      <div className="flex items-center gap-2">
        ...
        <h3 className="text-sm font-semibold text-foreground">Plant reports</h3>
      ...
      {loading ? (
        ...
        <div className="text-[11px] text-gray-600 border border-dashed border-card-border rounded-lg py-5 text-center flex flex-col items-center justify-center gap-2">
          <span>No reports yet. One is filed each time a plant is scanned.</span>
          <a href="/mission/report/123" target="_blank" rel="noopener noreferrer" className="text-primary hover:underline flex items-center gap-1 font-semibold">
            <ExternalLink className="w-3 h-3" />
            View Sample Pepper Report
          </a>
        </div>
      ) : (
            ...
            return (
              <li key={r.id}
                ...
                <a
                  ...
                  className="p-1.5 rounded hover:bg-white/5 text-muted-foreground hover:text-white shrink-0"
                >
                ...
                {confirming === r.id ? (
                  <div className="flex items-center gap-1 shrink-0">
                    ...
                    <button
                      ...
                      className="px-2 py-1 rounded bg-card-border hover:bg-gray-700 text-foreground text-[10px]"
                    >
                      Keep
                    </button>
                  </div>
                  ...
                  <button
                    ...
                    className="p-1.5 rounded hover:bg-rose-500/10 text-muted-foreground hover:text-rose-400 shrink-0"
                  >
