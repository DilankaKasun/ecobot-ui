        ...
        <dl className="grid grid-cols-1 sm:grid-cols-2 gap-x-6 gap-y-2">
          {rows.map(([k, v]) => (
            <div key={k} className="min-w-0">
              ...
              <dd className="text-sm text-gray-200 leading-snug">{v}</dd>
            </div>
          ))}
        </dl>
      )}
  ...
  return (
    <section className="bg-card border border-card-border rounded-xl p-5 space-y-2">
      ...
      <ul className="space-y-1.5">
        {items.map((t, i) => (
          <li key={i} className="flex gap-2 text-sm text-gray-200 leading-snug">
...
export default function ReportPage() {
  ...
  useEffect(() => {
    let live = true;
    const made: string[] = [];
    void getReport(String(params.id)).then((r) => {
      if (!live) return;
      setReport(r);
      setLoading(false);
      if (r) {
        r.photos.forEach((p) => made.push(URL.createObjectURL(p.blob)));
        setUrls(made);
      }
    });
    return () => {
  ...
  if (loading) {
    return <div className="p-8 text-sm text-gray-400">Opening the report…</div>;
  }
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-4">
      <Link href="/mission" className="inline-flex items-center gap-1.5 text-xs text-gray-400 hover:text-white">
        <ArrowLeft className="w-3.5 h-3.5" /> Back to the run
      </Link>
      ...
      {report.notes && (
        <p className="text-sm text-gray-200 bg-card border border-card-border rounded-xl p-5 leading-relaxed">
          {report.notes}
        </p>
      )}
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
            {urls.map((u, i) => (
              <figure key={u} className="space-y-1">
                ...
                <figcaption className="text-[10px] text-gray-500 font-mono truncate">
